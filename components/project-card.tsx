...
export default function ProjectCard({ title, desc, tags, image, slug }: ProjectProps) {
  ...
  return (
    <motion.div
      whileHover={{ y: -8 }}
      onHoverStart={() => mode === "super" && play("hover")}
      className="h-full"
    >
      <Link href={`/projects/${slug}`}>
        <Card 
          className={cn(
            "h-full overflow-hidden bg-surface border-muted/20 transition-all duration-300",
            // Super Mode Styles
            mode === "super" && "hover:border-accent-1 hover:shadow-[0_0_20px_rgba(0,225,255,0.3)] hover:scale-[1.02]"
          )}
        >
          {/* Image Container */}
          ...
          <CardHeader>
            <CardTitle className="text-xl text-foreground flex justify-between items-center">
              {title}
              {mode === "super" && <span className="text-xs text-accent-2 font-mono">[SECURE]</span>}
            </CardTitle>
          </CardHeader>
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                <Badge 
                  ...
                  className={cn(
                    "bg-background/50",
                    mode === "super" && "text-accent-2 border border-accent-2/20"
                  )}
                >
                  {tag}
                </Badge>
